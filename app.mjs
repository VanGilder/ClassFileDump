...
window.toggleProfileView = async function () {
    ...
    if (targetEl.getAttribute("data-toggle-state") === "collapsed") {
        targetEl.children[1].removeAttribute("hidden", true);
...
for (let user of alphabeticalUsersList) {
    ...
    html += `
        <div id=${user.id} class="card" onclick="toggleProfileView()">
            <div class="username">${user.fullName}</div>
            <div class="expanded-view" hidden></div>
        </div>
        <hr/>
            `.trim();
}
